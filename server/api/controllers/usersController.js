...
export default class {
  ...
  async info(req) {
    ...
    if ((!usernameEmail) || Object.keys(req.body).length > 1) {
      return { status: 'fail', statusCode: 400, message: 'Please enter username or email' };
    }
    if (usernameEmail) usernameEmail.trim().toLowerCase();
    const text = 'SELECT * FROM users WHERE email = $1 OR username = $1';
    try {
      ...
      const { userid, username, email, role, orders } = rows[0];
      const token = this.auth.generateToken(userid, username, email, role, orders);
      return {
        status: 'success', statusCode: 200, message: 'User details are as follows:', user: rows[0], token,
      };
    } catch (error) {
      return { status: 'fail', statusCode: 500, error: error.message };
    }
